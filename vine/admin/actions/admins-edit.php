...
// Level 1 admins can only edit their own profile
if ( ! $login->hasFullAccess() && $this->input('id') != $login->get('id')) {
    $this->setError('Access denied.');
}

// Support data
$status = implode(',', array_keys(Options::admins_status()));
$access = implode(',', array_keys(Options::admins_access()));
...
// Only applicable for full admins
...
if ($this->db->fetch($sql, $this->input('email'), $this->input('id'))) {
    $this->setError('email', 'Another admin has the same email address.');
}
...
// Update record
$this->db->update('admins', $data, 'id = ?', 1, $this->input('id'));

// Successful (level 1 admin)
if ( ! $login->hasFullAccess()) {
    $this->setStatus(TRUE);
    $this->setMessage('Profile successfully updated!');
    $this->setUrl('main');
    $this->finish();
// Successful (level 2 admin)
} else {
    $this->setStatus(TRUE);
    $this->setMessage('Admin edited successfully!');
