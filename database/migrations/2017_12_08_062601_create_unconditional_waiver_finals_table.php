...
class CreateUnconditionalWaiverFinalsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('unconditional_waiver_finals', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->bigInteger('project_id')->unsigned()->index()->unique();
            $table->string('CustomerName')->nullable();
            $table->string('ProjectName')->nullable();
            $table->text('ProjectAddress')->nullable();
            $table->decimal('ExtraAmount',8,2)->nullable();
    ...
    {
        Schema::dropIfExists('unconditional_waiver_finals');
    }
}
