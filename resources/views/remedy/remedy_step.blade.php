                        ...
                        <div class="main-tab-sec">
                            ...
                            <div class="tab-content tab-final">
                                <div id="public" class="tab-pane fade in active">
                                    <form method="post" action="{{ route('remedy.hide.remedy_steps') }}">
                                        {{ csrf_field() }}
                                        <button class="btn btn-danger" type="Submit" type="button">Private</button>
                                        <div class="box-header">
                                            <div class="row">
                                                <div class="col-md-7 col-sm-7">
                                                    <h3 class="box-title">List of all Remedy Steps</h3>
                                                </div>
                                       ...
                                       <a href="{{ route('remedy.get.remedy_step') }}" class="btn btn-danger">Reset</a>
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <table class="table table-striped">
                                                ...
                                                @if (count($remedySteps) > 0)
                                                    @foreach ($remedySteps as $key => $remedyStep)
                                                        <tr>
                                                            <td><input type="checkbox" name="hideRemedySteps[]"
                                                                    value="{{ $remedyStep->id }}" class="remstep"></td>
                                                            <td>{{ $key + 1 }}</td>
                                                            <td>{{ $remedyStep->getRemedy->state->name . ' / ' . $remedyStep->getRemedy->type->project_type . ' / ' . $remedyStep->getRemedy->remedy }}
                                                            </td>
                                                            <td>{{ $remedyStep->remedy_date->date_name }}</td>
                                                            <td>{{ $remedyStep->short_description }}</td>
                                                            <td>{{ $remedyStep->long_description }}</td>
                                                            <td>{{ $remedyStep->years }}</td>
                                                            <td>{{ $remedyStep->months }}</td>
                                                            <td>{{ $remedyStep->days }}</td>
                                                            <td>{{ $remedyStep->day_of_month }}</td>
                                                            <td>
                                                                {{ $remedyStep->notes }}
                                                            </td>
                                                            {{-- <td>
                                    <button class="btn btn-info btn-xs addRemedy"
                                       data-type="edit"
                                       type="button" title="Edit"><i class="fa fa-pencil"></i> </button>
                                    <button class="btn btn-danger btn-xs delete" type="button" data-id="{{ $remedyStep->id }}" title="Delete"><i class="fa fa-trash"></i> </button>
                                 </td> --}}
                                                        </tr>
                                                    @endforeach
                                                @else
                                        ...
                                        <div>
                                            ...
                                            <div class="box-footer clearfix">
                                                <ul class="pagination pagination-sm no-margin pull-right">
                                                    {{ $remedySteps->appends(request()->query())->links() }}
                                                </ul>
                                            </div>
                                        </div>
                                    </form>
                                ...
                                <div id="private" class="tab-pane fade">
                                    <form method="post" action="{{ route('remedy.hide.remedy_steps_private') }}">
                                        {{ csrf_field() }}
                                                                        ...
                                                                        @if ($state1->count() > 0)
                                                                            ...
                                                                            @foreach ($state1 as $state1)
                                                                                <option value="{{ $state1->id }}"
                                                                                    {{ $filter_private == $state1->id ? 'selected' : '' }}>
                                                                                    {{ $state1->name }}</option>
                                                                            @endForeach
                                                                        @else
                                                                            No Record Found
                                       ...
                                       <a href="{{ route('remedy.get.remedy_step') }}" class="btn btn-danger">Reset</a>
                                        ...
                                        <div class="box-body table-responsive no-padding">
                                            <table class="table table-striped">
                                                ...
                                                @if (count($remedyStepPrivate) > 0)
                                                    @foreach ($remedyStepPrivate as $key => $remedyStep)
                                                        <tr>
                                                            <td><input type="checkbox" name="hideRemedySteps[]"
                                                                    value="{{ $remedyStep->id }}" class="remstep_private">
                                                            </td>
                                                            <td>{{ $key + 1 }}</td>
                                                            <td>{{ $remedyStep->getRemedy->state->name . ' / ' . $remedyStep->getRemedy->type->project_type . ' / ' . $remedyStep->getRemedy->remedy }}
                                                            </td>
                                    ...
                                    <button class="btn btn-danger btn-xs delete" type="button" data-id="{{ $remedyStep->id }}" title="Delete"><i class="fa fa-trash"></i> </button>
                                        ...
                                        <div>
                                            ...
                                            <div class="box-footer clearfix">
                                                <ul class="pagination pagination-sm no-margin pull-right">
                                                    {{ $remedyStepPrivate->appends(request()->query())->links() }}
                                                </ul>
                                            </div>
                                        </div>
                                    </form>
    ...
    <script>
        ...
        $('#remedy_search_private').keyup(function(e){
            if(e.keyCode == 13)
            {
                ...
                var location = appendToQueryString('remedyStepPrivate', tier);
        ...
        $('#remedy_search').keyup(function(e){
            if(e.keyCode == 13)
            {
                ...
                var location = appendToQueryString('remedyStep', tier);
                window.location.href = location;
            }
        });
        ...
        $('.search').on('click', function() {
            ...
            var location = appendToQueryString('remedyStep', remedy);
            window.location.href = location;
        });
        ...
        $('.search_private').on('click', function() {
            ...
            var location = appendToQueryString('remedyStepPrivate', remedy);
            window.location.href = location;
        });
        ...
        $('#select_all').change(function() {
            if ($(this).is(':checked')) {
                $('.remstep').attr('checked', true);
            } else {
                $('.remstep').attr('checked', false);
            }
            // $('.remdate').attr('checked', true);
        });
        $('#select_all_private').change(function() {
            if ($(this).is(':checked')) {
                $('.remstep_private').attr('checked', true);
            } else {
                $('.remstep_private').attr('checked', false);
            }
            // $('.remdate').attr('checked', true);
        });
