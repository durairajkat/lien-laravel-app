                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">List of all Tier Remedy Steps</h3>
                            {{-- <button class="btn btn-md addTier" data-type="add" type="button">Add new Tier Remedy Steps</button> --}}
                        ...
                        <div class="box-body table-responsive no-padding">
                            <table class="table table-striped">
                                ...
                                @if (count($tierRemedySteps) > 0)
                                    @foreach ($tierRemedySteps as $key => $tierRemedyStep)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $tierRemedyStep->tier->tier_code }}</td>
                                            <td>{{ $tierRemedyStep->remedy_step->getRemedy->state->name }}</td>
                                            <td>{{ $tierRemedyStep->remedy_step->getRemedy->type->project_type }}</td>
                                            <td>{{ $tierRemedyStep->remedy_step->short_description }}</td>
                                            <td>{{ $tierRemedyStep->answer1 }}</td>
                                            <td>
                                                {{ $tierRemedyStep->answer2 }}
                                            </td>
                                            {{-- <td>
                                                <button class="btn btn-info btn-xs addRemedy"
                                                        data-type="edit"
                                                        type="button" title="Edit Remedy"><i class="fa fa-pencil"></i> </button>
                                                <button class="btn btn-danger btn-xs delete" type="button" data-id="{{ $tierRemedyStep->id }}" title="Delete Remedy"><i class="fa fa-trash"></i> </button>
                                            </td> --}}
                                        </tr>
                                    @endforeach
                                @else
                        ...
                        <div class="box-footer clearfix">
                            <ul class="pagination pagination-sm no-margin pull-right">
                                {{ $tierRemedySteps->appends(Request::except('page'))->links() }}
    ...
    <script>
        ...
        $('#remedy_search').keyup(function(e){
            if(e.keyCode == 13)
            {
                ...
                var location = appendToQueryString('tierRemedySearch', string);
        ...
        $('.search').on('click', function() {
            ...
            var location = appendToQueryString('tierRemedySearch', remedy);
            window.location.href = location;
        });
