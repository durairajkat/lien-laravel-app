...
@section('body')
    ...
    <div class="buttons-on-top row button-area">
        @if (!isset($_GET['edit']))
            <a href="javascript:void(0)" id="addContactsStep" class="project-create-continue">
                Contract Information
            </a>
        @else
            <a href="javascript:void(0)" id="skip-button-4-out" class="skip-contract-details project-create-skip">
                Skip
            </a>
            <a href="javascript:void(0)" id="addContactsStep-out" class="project-create-continue">
                Save & Continue
            </a>
        @endif
    </div>
    <div class="container-holder" style="display: block;">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <div class="border-table">
                        ...
                        <input type="text" name="job_name" value="{{ $project ? $project->project_name : '' }}"
                    ...
                    <div class="border-table">
                        ...
                        <input type="text" id="job_city" name="job_city" value="{{ $project ? $project->city : '' }}"
                            class="search-construction form-control input-field">
                    </div>
                    <div class="border-table">
                        ...
                        <select name="job_state" id="job_state" class="form-control input-field" disabled>
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}"
                                    {{ $project->state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="border-table">
                        ...
                        <input type="text" id="job_zip" name="job_zip" value="{{ $project ? $project->zip : '' }}"
                            class="search-construction form-control input-field">
                    </div>

                    <div class="border-table">
                        <label for="county">County</label>
                        <input type="text" id="job_county" name="county" value="{{ $project ? $project->county : '' }}"
                            class="search-construction form-control input-field">
                    </div>
    ...
    <div class="job-info-container create-project-form-bgcolor"
        style="width: 50%; display:none; height:auto; padding: 7% 5%;">
    ...
    <div class="save-skip flex items-center">
        @if (!isset($_GET['edit']))
            <a href="javascript:void(0)" id="addContactsStep" class="project-create-continue">
                Contract Information
            </a>
        @else
            <a href="javascript:void(0)" id="skip-button-4" class="skip">
                Skip
            </a>

            <a href="javascript:void(0)" id="addContactsStep" class="orange-btn">
                Save & Continue
            </a>
        @endif
    </div>
...
@section('script')
    ...
    <script>
        ...
        function initialize() {
            ...
            autocomplete.addListener('place_changed', function() {
                ...
                if ("{!! $project->state->name !!}" != state) {
                    alert("You can only choose address in " + "{!! $project->state->name !!}" + "! " + state +
                        " is selected!");
                    $('#job_address').val('');
                    return;
                }
        ...
        $('#job_address, #job_city, #job_zip').mousedown(function() {
            //console.log('clicked')
    ...
    <script>
        $(document).ready(function() {
            ...
            // skip buttons
            // 13-aug-2019
            $('#skip-button-4').on('click', function(event) {
    ...
    @php
    if (isset($_GET['edit'])) {
        $contactURL = route('project.contract.view') . '?project_id=' . $project->id . '&edit=true';
        $contractRoute = route('project.contract.view') . '?project_id=' . $project->id . '&edit=true';
        $jobDatesRoute = route('create.remedydates', $project->id) . '?edit=true';
    } else {
        $contactURL = route('project.contract.view') . '?project_id=' . $project->id;
        $contractRoute = route('project.contract.view') . '?project_id=' . $project->id . '&edit=true';
        $jobDatesRoute = route('create.remedydates', $project->id) . '?edit=true';
    }
    @endphp
    <script>
        ...
        let saveJobDescription = "{{ route('edit.job.description', [$project_id]) }}"
        let contactURL = "{{ $contactURL }}"
        let contractURL = "<?php echo $contractRoute; ?>"
        ...
        function getData(term, component) {
            $.ajax({
                ...
                success: function(data) {
                    $(".create-project-form-bgcolor").css({
                        "width": "50%",
                        "color": "white"
                    });
                    $(".ui-menu").css({
