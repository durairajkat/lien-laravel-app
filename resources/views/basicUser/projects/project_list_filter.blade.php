        ...
        <tr>
            ...
            <td>
                ...
                <select class="form-control chosen" name="user">
                    <option value="all"
                        @if (isset($queryParams) && array_key_exists('user', $queryParams) && $queryParams['user'] == 'all')
                            {{'selected'}}
                        @endif
                    >All Users</option>
                    @if (isset($subUsers))
                        @forelse($subUsers as $key => $subUser)
                            <option value="{{$key}}"
                                @if (isset($queryParams) && array_key_exists('user', $queryParams) && $queryParams['user'] == $key)
                                    {{'selected'}}
                                @endif
                            >{{$subUser}}</option>
                        @empty
        ...
        <tr>
            <td>
                ...
                <select class="form-control chosen" name="date_type">
                    <option value="all" @if (isset($queryParams) && array_key_exists('dateType', $queryParams) && $queryParams['dateType'] == 'all')
                        ...
                        >All</option>
                    @isset($dateTypes)
                        @forelse($dateTypes as $key => $dateType)
                            <option value="{{ $key }}" @if (isset($queryParams) && array_key_exists('dateType', $queryParams) && $queryParams['dateType'] == $key)
                                {{ 'selected' }}
                        @endif
                        >{{ $dateType }}</option>
                    @empty
            ...
            <td style="width: 20%">
                ...
                <select class="form-control chosen" name="calculation_status">
                    <option value="" @if (isset($queryParams) && array_key_exists('calculation_status', $queryParams) && $queryParams['calculation_status'] == '')
                        {{ 'selected' }}
                        @endif
                        >All</option>
                    <option value="0" @if (isset($queryParams) && array_key_exists('calculation_status', $queryParams) && $queryParams['calculation_status'] == '0')
                        {{ 'selected' }}
                        @endif
                        >In Process</option>
                    <option value="1" @if (isset($queryParams) && array_key_exists('calculation_status', $queryParams) && $queryParams['calculation_status'] == '1')
                        {{ 'selected' }}
                        @endif
                        >Complete</option>
                </select>
